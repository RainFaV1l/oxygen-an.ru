<template>
  <header class="flex flex-col items-center pb-16">
    <div class="container py-5 flex flex-wrap justify-between items-center">
      <div class="w-10 h-10 sm:w-16 sm:h-16 flex flex-col justify-center items-center gap-3 sm:gap-4 cursor-pointer" @click="setIsModalMenuOpen(true)">
        <div class="w-full border border-dark rounded-full"></div>
        <div class="w-full border border-dark rounded-full"></div>
      </div>
      <a class="ml-9 h-12 font-medium cursor-pointer" @click="$router.push(`/`)">
    ...
    <div class="h-24 mr-11">
      <img class="h-full w-full object-contain" :src="bgLogo" alt="logo">
    </div>
  </header>
</template>
